...
for _ in tqdm(range(105), desc='Processing: ', colour='blue'):
    ...
    for counter in range(num_realizations):
        ...
        for j in y:
            if j == 0:
                ...
                q = np.arange(0)
            q = np.append(q, j)
        coord = np.append(coord, amplitudes)
        if extremas < amplitudes: extremas = amplitudes
        rho_array = np.append(rho_array, amplitudes / extremas)
        amplitudes, extremas = 0, 0
    rho = np.mean(rho_array)
    ...
    x = np.append(x, int(np.mean(coord)))
    N += 25
...
np.save('x_approaches', x)
np.save('eps_experimental', eps_experimental)
np.save('eps_spectral', eps_spectral)
np.save('eps_theoretical', eps_theoretical)
ax.set_xlabel('Number of individual waves', fontsize=20)
