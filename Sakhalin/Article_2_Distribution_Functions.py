...
ax[0].plot(x1, y1, linewidth=2, marker='.', alpha=.65,
           color=colors[0], label=r"$BFI_\infty$" + f' ∈ [{np.round(p0, 2)};{np.round(p1, 2)})')
ax[0].plot(x2, y2, linewidth=2, marker='.', alpha=.65,
           color=colors[1],
           label=r"$BFI_\infty$" + f' ∈ [{np.round(p1, 2)};{np.round(p2, 2)})')
ax[0].plot(x3, y3, linewidth=2, marker='.', alpha=.65,
           color=colors[2], label=r"$BFI_\infty$" + f' ∈ [{np.round(p2, 2)};{np.round(p3, 2)})')
ax[0].plot(x4, y4, linewidth=2, marker='.', alpha=.65,
           color=colors[3], label=r"$BFI_\infty$" + f' ∈ [{np.round(p3, 2)};{np.round(p4, 2)}]')
ax[0].set_ylim(top=1, bottom=np.min(np.array([np.min(y1), np.min(y2), np.min(y3), np.min(y4)])))
ax[0].legend(fontsize=21)
...
ax[1].plot(x1, y1, linewidth=2, marker='.', alpha=.65,
           color=colors[0], label=r"$BFI$" + f' ∈ [{np.round(p0, 2)};{np.round(p1, 2)})')
ax[1].plot(x2, y2, linewidth=2, marker='.', alpha=.65,
           color=colors[1],
           label=r"$BFI$" + f' ∈ [{np.round(p1, 2)};{np.round(p2, 2)})')
ax[1].plot(x3, y3, linewidth=2, marker='.', alpha=.65,
           color=colors[2], label=r"$BFI$" + f' ∈ [{np.round(p2, 2)};{np.round(p3, 2)})')
ax[1].plot(x4, y4, linewidth=2, marker='.', alpha=.65,
           color=colors[3], label=r"$BFI$" + f' ∈ [{np.round(p3, 2)};{np.round(p4, 2)}]')
