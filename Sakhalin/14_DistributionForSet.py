...
splitted_all_p = split_array(all_p, np.array([len(all_p) // 3, len(all_p) // 3, len(all_p) // 3 + len(all_p) % 3]))
p0 = splitted_all_p[0][0]
p1 = splitted_all_p[0][-1]
p2 = splitted_all_p[2][0]
p3 = splitted_all_p[2][-1]
hight1 = np.arange(0)
hight2 = np.arange(0)
hight3 = np.arange(0)
...
xrg = np.arange(0, np.max(np.array([np.max(x1), np.max(x2), np.max(x3)])), 0.0001)
yr = np.exp(-2 * xrg * xrg)
c = np.sqrt(np.pi / 8)
...
ax.set_xlim(left=0)
ax.set_ylim(top=1, bottom=np.min(np.array([np.min(y1), np.min(y2), np.min(y3)])) - 10e-7)
ax.grid()
plt.yscale('log')
